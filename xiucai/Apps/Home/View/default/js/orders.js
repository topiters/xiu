...
function submitOrder(){
	...
	jQuery.post(Think.U('Home/Course/checkCourseStock') ,{},function(data) {
		...
		for(var i=0;i<courseInfo.length;i++){
			...
			if(course.isSale<1){
				WST.msg('课程'+course.courseName+'已下架，请返回重新选购!', {icon: 5});
				return;
			}else if(course.courseStock<course.cnt){
				WST.msg('课程'+course.courseName+'库存不足，请返回重新选购!', {icon: 5});
				return;
			}else if(course.shopAtive==0){
				WST.msg('商铺'+course.shopName+'在休息中，不能下单!', {icon: 5});
				return;
			}				
