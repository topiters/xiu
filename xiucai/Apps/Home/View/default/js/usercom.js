...
function addGoodsAppraises(shopId,goodsId,goodsAttrId,orderId){
	...
	//上传的图片
	var img = [];
	$('.appraise_pic').each(function(){
		img.push($(this).attr('src').replace(WST.DOMAIN+"/",""));
	});
	appraiseAnnex = img.join(',');


	
	//layer.confirm('您确定要提交该评价吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
		$.post(Think.U('Home/GoodsAppraises/addGoodsAppraises'),{shopId:shopId, goodsId:goodsId, goodsAttrId:goodsAttrId,orderId:orderId, goodsScore:goodsScore, timeScore:timeScore, serviceScore:serviceScore, content:content, appraiseAnnex:appraiseAnnex },function(data,textStatus){
			//layer.close(tips);
			layer.close(ll);
			var json = WST.toJson(data);
			if(json.status==1){
