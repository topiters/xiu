...
function goToPay(){
	...
	params.id=$('#courseIdArray').attr('data');
	...
	jQuery.post( Think.U('Home/Cart/checkCartCourseStock') ,params,function(data) {
...
//删除购物车中的单课程
function delCatCourse(id){
	var params={};
	//params.id=$('#courseIdArray').attr('data');
	//alert($('#courseIdArray').attr('data'));
	layer.confirm('您确定要从购物车中删除该课程吗？',{icon: 3, title:'系统提示',offset: '150px'}, function(tips){
		var ll = layer.load('数据处理中，请稍候...');
		
		jQuery.post(Think.U('Home/Cart/deleteCartCourse') ,{'id':id},function(data) {
			layer.close(ll);
	    	layer.close(tips);
			
			if(data.status==1){
				
					WST.msg('该课程删除成功',{offset: '200px'});
...
function delAllCourse(){
	...
	layer.confirm('您确定要从购物车中删除该课程吗？',{icon: 3, title:'系统提示',offset: '150px'}, function(tips){
		...
		jQuery.post(Think.U('Home/Cart/deleteCartCourse') ,params,function(data) {
			layer.close(ll);
	    	layer.close(tips);
